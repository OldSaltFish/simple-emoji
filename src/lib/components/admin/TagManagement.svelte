<script lang="ts">
  ...
  let editingTag = $state<Partial<Tag>>({});
  ...
  async function loadKinds() {
    try {
      kinds = await imagesApi.getTagKinds();
    } catch (error) {
  ...
  async function saveTag() {
    ...
    try {
      if (isNew) {
        await imagesApi.createTag(editingTag as any);
        showMessage('创建图片标签成功', 'success');
      } else {
        await imagesApi.updateTag(editingTag.name!, editingTag);
        showMessage('更新图片标签成功', 'success');
      }
      await Promise.all([loadTags(), loadKinds()]);
      showEditModal = false;
    } catch (error) {
  ...
  async function deleteTag(name: string) {
    if (await customConfirm(`确定要删除图片标签 "${name}" 吗？`)) {
      try {
        await imagesApi.deleteTag(name);
  ...
  // 获取合并后的分类建议
  let allKindOptions = $derived([...new Set([...kinds, ...TAG_KIND_OPTIONS])]);
</script>

<div class="bg-white rounded-lg shadow">
  <div class="p-6 border-b border-gray-200 flex justify-between items-center flex-wrap gap-4">
    <div class="flex items-center gap-4">
      <h2 class="text-2xl font-bold text-gray-900">图片标签管理</h2>
  ...
  <div class="p-6">
    <p class="text-sm text-gray-500 mb-6">管理用于图片/表情包分类的标签，支持按人物、情绪、风格等维度进行管理。</p>
...
{#if showEditModal}
  <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
    <div class="bg-white rounded-xl shadow-2xl max-w-md w-full p-6">
      <h3 class="text-xl font-bold text-gray-900 mb-6">{isNew ? '添加图片标签' : '编辑标签信息'}</h3>
      <div class="space-y-5">
        <div>
